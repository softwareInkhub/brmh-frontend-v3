...
export default function AllMethodPage({ namespace, onViewMethod, openCreate = false, refreshSidePanelData }: { namespace?: any, onViewMethod?: (method: any, ns?: any) => void, openCreate?: boolean, refreshSidePanelData?: () => Promise<void> }) {
  ...
  return (
    <div ref={pageRef} className="p-4 w-full flex relative bg-white dark:bg-gray-950">
      <div className="flex-1 pr-0">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">All Methods</h2>
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 w-full sm:w-auto">
            <div className="relative flex-1 sm:flex-initial">
              <input
                type="text"
                placeholder="Search methods..."
                className="pl-8 pr-3 py-1.5 sm:py-2 border border-gray-300 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 w-full text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400"
                value={search}
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              <button
                ...
                className={`px-2 py-1 rounded text-sm ${viewMode === 'grid' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                title="Grid view"
              >
                ▦
              </button>
              <button
                ...
                className={`px-2 py-1 rounded text-sm ${viewMode === 'list' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                title="List view"
              >
                ≡
              </button>
            ...
            <button
              className="inline-flex items-center justify-center gap-1 sm:gap-2 bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded text-sm sm:text-base"
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 ">
            {filteredMethods.map((m, idx) => {
              let typeIcon = <Database size={16} className="text-gray-400 dark:text-gray-500" />;
              if (m['namespace-method-type'] === 'GET') typeIcon = <Zap size={16} className="text-green-500 dark:text-green-400" />;
              if (m['namespace-method-type'] === 'POST') typeIcon = <Send size={16} className="text-orange-500 dark:text-orange-400" />;
              ...
              const typeBgClass = m['namespace-method-type'] === 'GET'
                ? 'bg-green-50 dark:bg-green-900/30 ring-green-200 dark:ring-green-800'
                : m['namespace-method-type'] === 'POST'
                ? 'bg-orange-50 dark:bg-orange-900/30 ring-orange-200 dark:ring-orange-800'
                : m['namespace-method-type'] === 'PUT'
                ? 'bg-blue-50 dark:bg-blue-900/30 ring-blue-200 dark:ring-blue-800'
                : m['namespace-method-type'] === 'DELETE'
                ? 'bg-red-50 dark:bg-red-900/30 ring-red-200 dark:ring-red-800'
                : 'bg-gray-50 dark:bg-gray-800 ring-gray-200 dark:ring-gray-700';
              return (
                <div
                  key={m['namespace-method-id'] || idx}
                  className="group relative rounded-xl border border-gray-100 dark:border-gray-700 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm px-5 py-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer"
                  ...
                  <div className="flex items-start gap-3 ">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400 truncate mt-1">
                        <span className="text-gray-400 dark:text-gray-500 mr-1">URL:</span>
                        <span className="font-medium text-gray-700 dark:text-gray-300" title={m['namespace-method-url-override'] || ''}>{m['namespace-method-url-override'] || <span className='italic text-gray-400 dark:text-gray-500'>No URL override</span>}</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm rounded-xl border border-gray-100 dark:border-gray-700 overflow-hidden shadow-sm">
            <div className="divide-y divide-gray-200 dark:divide-gray-700">
              {filteredMethods.map((m, idx) => {
                ...
                const typeBgClass = m['namespace-method-type'] === 'GET'
                  ...
                  : 'bg-gray-50 dark:bg-gray-800 ring-gray-200 dark:ring-gray-700';
                return (
                <div key={m['namespace-method-id'] || idx} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors cursor-pointer" onClick={() => setSidePanel({ method: m })}>
                  <div className="min-w-0 flex-1">
                    <div className="font-medium text-gray-900 dark:text-gray-100 truncate flex items-center gap-2">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold ring-1 ${typeBgClass} ${typeTextClass}`}>{m['namespace-method-type']}</span>
                      <span className="truncate">{m['namespace-method-name']}</span>
                    </div>
                    <div className="text-xs text-gray-500 dark:text-gray-400 truncate mt-0.5">URL: <span className="font-medium text-gray-700 dark:text-gray-300" title={m['namespace-method-url-override'] || ''}>{m['namespace-method-url-override'] || <span className='italic text-gray-400 dark:text-gray-500'>No URL override</span>}</span></div>
                  </div>
                  <div className="flex items-center gap-2 ml-auto">
                    <button className="text-green-600 dark:text-green-400 hover:text-green-800 dark:hover:text-green-300 p-1" title="Edit" onClick={(e) => { e.stopPropagation(); onViewMethod && onViewMethod({ ...m, __openEdit: true }, m.namespace); }}><Pencil size={16} /></button>
                    <button className="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 p-1" title="Delete" onClick={() => handleDelete(m['namespace-method-id'])}><Trash2 size={16} /></button>
                  </div>
                </div>
              )})}
            </div>
      ...
      {sidePanel === 'create' && (
        <div
          ...
          style={{ minHeight: '100vh', width: typeof window !== 'undefined' && window.innerWidth < 768 ? '100vw' : (sidePanel ? sidePanelWidth : 0), transform: sidePanel ? 'translateX(0)' : `translateX(${sidePanelWidth}px)`, boxShadow: sidePanel ? '0 0 32px 0 rgba(0,0,0,0.10)' : 'none', borderTopLeftRadius: 16, borderBottomLeftRadius: 16, overflow: 'auto' }}
        >
          <div
